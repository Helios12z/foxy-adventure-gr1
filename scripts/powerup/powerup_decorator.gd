...
func get_max_jump_count() -> int:
	if data.max_jump_count > 0:
		return data.max_jump_count

	return _get_next_max_jump_count()
...
func _get_next_max_jump_count() -> int:
	if next_decorator:
		return next_decorator.get_max_jump_count()
	return player.max_jump_count
