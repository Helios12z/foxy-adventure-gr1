extends PlayerState

var is_attacking := false
var attack_active_timer := 0.0
var attack_cooldown_timer := 0.0

const ATTACK_ACTIVE := 0.15
const ATTACK_COOLDOWN := 0.2
const ATTACK_OFFSET := 15

var direction_node: Node2D
var original_offset := Vector2.ZERO


func _enter() -> void:
	obj.change_animation("fall")

	is_attacking = false
	attack_active_timer = 0.0
	attack_cooldown_timer = 0.0
	obj.set_hit_collision(false)

	# lấy Direction node (quan trọng)
	direction_node = obj.get_node("Direction")
	original_offset = direction_node.position


func _update(delta: float) -> void:
	# --------------------------------------------------
	# COOLDOWN COUNTDOWN
	# --------------------------------------------------
	if attack_cooldown_timer > 0.0:
		attack_cooldown_timer -= delta


	# --------------------------------------------------
	# ATTACK ACTIVE COUNTDOWN
	# --------------------------------------------------
	if is_attacking:
		attack_active_timer -= delta

		if attack_active_timer <= 0.0:
			is_attacking = false
			obj.set_hit_collision(false)
			obj.change_animation("fall")

			# reset Direction
			direction_node.position = original_offset

		return


	# --------------------------------------------------
	# START ATTACK + PRE-TURN + OFFSET
	# --------------------------------------------------
	if Input.is_action_just_pressed("attack") \
	and attack_cooldown_timer <= 0.0 \
	and obj.has_blade:

		var input_dir := Input.get_axis("left", "right")
		if abs(input_dir) > 0.1:
			obj.change_direction(sign(input_dir))

		obj.attack_sound.play()
		is_attacking = true
		attack_active_timer = ATTACK_ACTIVE
		attack_cooldown_timer = ATTACK_COOLDOWN

		obj.change_animation("attack")
		obj.set_hit_collision(true)

		# offset Direction 5px
		if obj.is_right():
			direction_node.position.x = original_offset.x + ATTACK_OFFSET
		else:
			direction_node.position.x = original_offset.x - ATTACK_OFFSET

		return


	# --------------------------------------------------
	# SKILLS
	# --------------------------------------------------
	if control_dash(): return
	if control_susanoo(): return
	if control_room(): return
	if control_water_paw(): return
	if control_hover(): return


	# --------------------------------------------------
	# MOVEMENT
	# --------------------------------------------------
	control_jump()
	var is_moving := control_moving(delta)


	# --------------------------------------------------
	# WALL SLIDE
	# --------------------------------------------------
	if obj.can_wall_slide():
		var input_dir := Input.get_axis("left", "right")
		if (obj.is_on_left_wall() and input_dir < 0) \
		or (obj.is_on_right_wall() and input_dir > 0):
			change_state(fsm.states.climb)
			return


	# --------------------------------------------------
	# LANDING
	# --------------------------------------------------
	if obj.is_on_floor():
		if is_moving:
			change_state(fsm.states.walk)
		else:
			change_state(fsm.states.idle)
		return


func _exit() -> void:
	obj.set_hit_collision(false)
	if direction_node:
		direction_node.position = original_offset
