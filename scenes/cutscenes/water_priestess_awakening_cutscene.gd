...
func _on_trigger_area_body_entered(body: Node2D) -> void:
	...
	if body.is_in_group("player") or body.is_in_group("Player"):
		...
		if Dialogic.VAR.has("WaterPriestessCutsceneSeen") and Dialogic.VAR.get("WaterPriestessCutsceneSeen"):
			return
...
func _show_dialogue() -> void:
	# Pause boss during dialogue
	if boss:
		boss.in_dialogue = true

	# Mark cutscene as seen
	Dialogic.VAR.set("WaterPriestessCutsceneSeen", true)

	# Start Dialogic timeline
	Dialogic.timeline_ended.connect(_on_dialogue_ended)
	Dialogic.start("water_priestess_awakening")
...
func _end_cutscene() -> void:
	...
	# Trigger boss to start fighting
	_trigger_boss_start()

func _trigger_boss_start() -> void:
	if boss:
		boss.in_dialogue = false
		boss.seen_player = true
		boss.set_physics_process(true)  # Re-enable boss physics processing
		if boss.phase_1:
			boss.phase_1.play()
